...
def main():
    ...
    try:
        if len(OptionBasePlan.strip()) > 0:
            strName, strPath = mbhPtxPlanImport.findBasePlan(OptionBasePlan)
            if not strName:
                say("Could not find base plan: %s." % OptionBasePlan)
                return
        else:
            strName, strPath = mbhPtxPlanImport.findProjectProgress(Project)
            if not strName:
                say("Could not find ProjectProgress for: %s." % Project)
                return
            
        aBasePlan = mbhPtxPlanImport.ProjectPlan(strName, strPath, OptionLanguage)
        aBasePlan.parseAll()
